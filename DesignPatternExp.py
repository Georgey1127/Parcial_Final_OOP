...
Chapter 11. Common Design Patterns - State Design Pattern
"""
from __future__ import annotations
from typing import Optional, Iterable, Iterator, cast

                                                      #They are using the State design pattern for this code.
class NMEA_State:
  """
  This is the superclass pattern that will be invoked by other classes for its methods. 
  """
...
class Waiting(NMEA_State): 
  """
  This is a class which is invoking methods from the superclass's pattern.
  """
    def feed_byte(self, input: int) -> NMEA_State:
        if input == ord(b"$"):
...
class Header(NMEA_State):
  """
  This is also a class which is invoking methods from the superclass's pattern.
  """
    def __init__(self, message: "Message") -> None:
        self.message = message
        self.message.reset()
...
class Message:
  """
  This is a separated class that will have the methods for the message.
  """
...
class Reader:
  """
  This is a separated class for the read method. 
  """
    def __init__(self) -> None:
        self.buffer = Message()
